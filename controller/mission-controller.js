...
const createNewMission = async(req, res) => {
    try {
        // attributs
        const { description, comment, start, finish, destination, planeId, hotelId, clientId, employeeId } = req.body;
        // check client exist
        const checkClient = await db.client.findOne({ where: { id: clientId, activated: true } });
        if (!checkClient) {
            return res.status(404).json({ error: "client doesn't exist", code: "client" })
        }
        ...
        if (checkEmployee && checkEmployee.activated === false) {
            return res.status(409).json({ error: "employee is not activated", code: "activated" })
        }
        // check passport
        if (checkEmployee.currentPassport === null) {
            return res.status(409).json({ error: "invalid passport", code: "passport" })
        }
        // get employee current passport
        const currentPassport = await db.passport.findOne({where:{id:checkEmployee.currentPassport}})
        // check nationalite
        ...
        // 
        if (checkMission) {
            return res.status(409).json({ error: "mission already taken in that periode", code: "date" })
        }
        // create
...
const updateOneMission = async(req, res) => {
    try {
        ...
        const { id } = req.params;
        // attributs
        const { description, comment, start, finish, destination, planeId, hotelId } = req.body;
        // check if the mission exist
        const checkMission=await db.mission.findByPk(id);
        if (checkMission) {
            return res.status(404).json({ error: "mission does't exist", code: "mission" })
        }
        //checks
        ...
        // update
        const updateMission = await db.mission.update({ description, comment, start, finish, destination, planeId, hotelId }, { where: { id } });
        if (!updateMission) {
...
const setMissionStatus=async(req,res)=>{
    try {
        ...
        if(checkMission.accepted===true){
            return res.status(409).json({message:"mission already accepted",code:"accepted"})
        }
        // check if its already declined
        else if(checkMission.accepted===true){
            return res.status(409).json({message:"mission already declined",code:"declined"})
        }
        let changes={};
        // accept it
        if(operation==="accept"){
        ...
        // declined
            ...
            changes={accepted:false,declinedAt:Date.now(),declined:true}
        }
        // update
        ...
        // 
        return res.status(202).json({message:operation==="accept"?"mission accepted successfully":"mission declined successfully"})
    } catch (error) {
        console.log("erro: ", error)
        return res.status(500).json({ error: "server error" })
    }
...
const retriveOneMission = async(req, res) => {
    try {
        ...
        const { id } = req.params;
        // retrive
        const item = await db.mission.findByPk(id, {
                include: [{
                    model: db.client,
